...
class NodoProcesadorImagen(Node):
    ...
    def callback_procesamiento_imagen(self, msg_imagen_ros):
        ...
        try:
            ...
            correccion_angulo_publicar = 0.0 # Nuevo: -1 girar izq, 0 alineado, 1 girar der
            ...
            if num_valid_targets_publicar > 0:
                ...
                #Cálculo de corrección de ángulo
                rect_min_area_obj = closest_target['rect_obj'] # ((cx,cy), (w,h), angulo)
                ...
                if angulo_corregido_deg > UMBRAL_ANGULO_DEG:
                    ...
                    correccion_angulo_publicar = 1.0  # Señal para girar a la derecha

                # Dibujar sobre img_proc (que es RGB)
                points = cv2.boxPoints(rect_min_area_obj) # Usar el objeto ya obtenido
                ...
                # Actualizar texto con info de ángulo (opcional, pero útil para depurar)
                # info_text = f"D:{distancia_m_publicar:.1f}m X:{offset_x_norm_publicar:.2f} Y:{offset_y_norm_publicar:.2f} A:{angulo_corregido_deg:.1f} C:{correccion_angulo_publicar}"
                info_text = f"D:{distancia_m_publicar:.1f}m X:{offset_x_norm_publicar:.2f} Y:{offset_y_norm_publicar:.2f}" # Mantenerlo simple por ahora
            ...
            # Formato: [offset_x_normalizado, offset_y_normalizado, distancia_metros, numero_objetivos_validos, correccion_angulo_publicar]
            msg_datos_deteccion.data = [
                offset_x_norm_publicar,
                offset_y_norm_publicar,
                distancia_m_publicar,
                float(num_valid_targets_publicar),
                correccion_angulo_publicar # Añadido el nuevo valor
            ]
