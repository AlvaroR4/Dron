        ...
        if contours:
            ...
            if contours: 
                ...
                if M['m00'] > 100: 
                    ...
                    self.get_logger().debug(f'Target found. Offset: {offset_x},{offset_y}. Area: {contour_area:.0f}')

                    cv2.putText(imgRGB, f"x:{offset_x},y:{offset_y},A:{contour_area:.0f}", (cX - 50, cY - 20),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        if not found_target:
             self.get_logger().debug('No target found. Sending offset 0,0, area 0.')
             offset_x = 0
             offset_y = 0
             contour_area = 0.0

        message = f"{offset_x},{offset_y},{contour_area}"
        try:
